...
{
    /**
     * All Handlers associated with server.Server.
     * Each server.Handler is associated with a Client connected to the server.Server.
     */
    ...
    /**
     * server.Server's port number
     */
    int portNumber;
    /**
     * Status of server.Server
     */
    boolean open = true;
    ...
    public Server(int portNumber, JTextArea messageArea) throws IOException
    {
        ...
        this.setName("server.Server" + portNumber);
    }
     ...
     * @param h the server.Handler associated to the client
     */
    ...
    /**
     * Carries out key exchange for all Clients with associated server.ServerKeyExchange
     * @param i x-coordinate of associated server.ServerKeyExchange
     * @param j y-coordinate of associated server.ServerKeyExchange
     * @throws IOException
    ...
    public void doExchange(int i, int j) throws IOException, InterruptedException
    {
        ...
        // Makes Sockets and ServerExchangeThreads for each server.Handler/Client connected
        int c = 0;
        ...
        // Create server.ExchangeTree, start ExchangeThreads, do the key exchange
    ...
    public void run()
    {
        ...
        // Wait for all values to be generate
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
